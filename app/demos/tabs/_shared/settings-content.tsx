      ...
      <div className="space-y-3">
        {fields.map((f) => (
          <div key={f.label} className="space-y-1.5">
            ...
            <div className="h-9 rounded-md border border-zinc-700 bg-zinc-800 px-3 flex items-center text-sm text-zinc-50">
              {f.value}
            </div>
          </div>
        ))}
        <div className="flex items-center justify-between rounded-md border border-zinc-700 bg-zinc-800 px-3 py-2">
          <div className="text-sm text-zinc-50">Email me on new errors</div>
          <div className="h-5 w-9 rounded-full bg-zinc-200 relative">
            <div className="absolute right-0.5 top-0.5 size-4 rounded-full bg-zinc-900" />
          </div>
        </div>
        <button className="h-9 rounded-md bg-zinc-50 text-zinc-900 px-4 text-sm font-medium">
          Save changes
        </button>
      </div>
    </div>
