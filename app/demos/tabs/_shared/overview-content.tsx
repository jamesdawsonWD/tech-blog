      ...
      <div className="grid grid-cols-3 gap-3">
        {metrics.map((m) => (
          <div key={m.label} className="rounded-lg border border-zinc-200 bg-white p-4">
            ...
            <div className="mt-2 text-2xl font-semibold tabular-nums text-zinc-900">{m.value}</div>
            <div className="mt-1 text-xs text-emerald-600">{m.delta}</div>
          </div>
        ))}
      </div>
      <div className="rounded-lg border border-zinc-200 bg-white p-4">
        <div className="text-xs text-zinc-500 mb-3">Requests (last 24h)</div>
