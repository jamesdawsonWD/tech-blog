// LCP Quick Check
// Source: https://webperf-snippets.nucliweb.net/CoreWebVitals/LCP
...
(() => {
  ...
  const observer = new PerformanceObserver((list) => {
    ...
    if (element) {
      ...
      if (tagName === "img") {
        ...
        if (element.naturalWidth) console.log(`   Dimensions: ${element.naturalWidth}×${element.naturalHeight}`);
      ...
      if (lastEntry.size) console.log(`   Size: ${lastEntry.size.toLocaleString()} px²`);
  ...
  if (!lastLcpEntry) return { script: "LCP", status: "error", error: "No LCP entries yet" };
  ...
  return {
    script: "LCP", status: "ok", metric: "LCP", value: lcpValue, unit: "ms", rating: lcpRating,
    thresholds: { good: 2500, needsImprovement: 4000 },
    details: { element: lcpSelector, elementType: lcpType, url: lastLcpEntry.url || null, sizePixels: lastLcpEntry.size || null },
  };
})();
