---
title: "Create a newsletter site for free in 30 minutes"
description: "We will create a Newsletter website in Nuxt in less than 30 minutes!"
date: "2024-07-01"
author:
  name: "James Dawson"
  avatar: "/avatar.jpg"
coverImage: "/resendandnuxt.png"
tags: ["nuxt", "resend", "web development"]
components: ["CodeBlock", "CodeGroup"]

---

## Initial Setup

We need to create a new project.

<CodeGroup
  options={[
    {
      name: "pnpm",
      lang: "bash",
      code: [
        "pnpm dlx nuxi@latest init <project-name>",
        "pnpm add -D typescript",
        "pnpm dlx nuxi@latest module add @nuxtjs/tailwindcss",
        "pnpm dlx nuxi@latest module add shadcn-nuxt",
      ].join("\n\n"),
    },
    {
      name: "npm",
      lang: "bash",
      code: [
        "npx nuxi@latest init <project-name>",
        "npm install -D typescript",
        "npx nuxi@latest module add @nuxtjs/tailwindcss",
        "npx nuxi@latest module add shadcn-nuxt",
      ].join("\n\n"),
    },
    {
      name: "yarn",
      lang: "bash",
      code: [
        "yarn dlx nuxi@latest init <project-name>",
        "yarn add -D typescript",
        "yarn dlx nuxi@latest module add @nuxtjs/tailwindcss",
        "yarn dlx nuxi@latest module add shadcn-nuxt",
      ].join("\n\n"),
    },
    {
      name: "bun",
      lang: "bash",
      code: [
        "bunx nuxi@latest init <project-name>",
        "bun add -d typescript",
        "bunx nuxi@latest module add @nuxtjs/tailwindcss",
        "bunx nuxi@latest module add shadcn-nuxt",
      ].join("\n\n"),
    },
  ]}
/>

Then we open our `nuxt.config.ts` file. And replace it with the following.

<CodeBlock name="tailwind.config.js" lang="js">
  {`
export default defineNuxtConfig({
  modules: ["@nuxtjs/tailwindcss", "shadcn-nuxt"],
  shadcn: {
    /**
     * Prefix for all the imported component
     */
    prefix: "",
    /**
     * Directory that the component lives in.
     * @default "./components/ui"
     */
    componentDir: "./components/ui",
  },
});
  `}
</CodeBlock>

Run the setup CLI.

<CodeGroup
  options={[
    {
      name: "pnpm",
      lang: "bash",
      code: [
        "pnpm dlx nuxi@latest init <project-name>",
        "pnpm add -D typescript",
        "pnpm dlx nuxi@latest module add @nuxtjs/tailwindcss",
        "pnpm dlx nuxi@latest module add shadcn-nuxt",
        "pnpm dlx shadcn-vue@latest init",
      ].join("\n\n"),
    },
    {
      name: "npm",
      lang: "bash",
      code: [
        "npx nuxi@latest init <project-name>",
        "npm install -D typescript",
        "npx nuxi@latest module add @nuxtjs/tailwindcss",
        "npx nuxi@latest module add shadcn-nuxt",
        "npx shadcn-vue@latest init",
      ].join("\n\n"),
    },
    {
      name: "yarn",
      lang: "bash",
      code: [
        "yarn dlx nuxi@latest init <project-name>",
        "yarn add -D typescript",
        "yarn dlx nuxi@latest module add @nuxtjs/tailwindcss",
        "yarn dlx nuxi@latest module add shadcn-nuxt",
        "yarn dlx shadcn-vue@latest init",
      ].join("\n\n"),
    },
    {
      name: "bun",
      lang: "bash",
      code: [
        "bunx nuxi@latest init <project-name>",
        "bun add -d typescript",
        "bunx nuxi@latest module add @nuxtjs/tailwindcss",
        "bunx nuxi@latest module add shadcn-nuxt",
        "bunx shadcn-vue@latest init",
      ].join("\n\n"),
    },
  ]}
/>

And finally just follow the default steps. _Make sure you select your framework as Nuxt_

<CodeBlock name="nuxt setup" lang="txt">
  {`
Would you like to use TypeScript (recommended)?  yes
Which framework are you using? Nuxt
Which style would you like to use? › Default
Which color would you like to use as base color? › Slate
Where is your tsconfig.json or jsconfig.json file? › ./tsconfig.json
Where is your global CSS file? › › src/index.css
Do you want to use CSS variables for colors? › no / yes
Where is your tailwind.config.js located? › tailwind.config.js
Configure the import alias for components: › @/components
Configure the import alias for utils: › @/lib/utils
Write configuration to components.json. Proceed? > Y/n
  `}
</CodeBlock>

That is all the setup done! All we need to do now is add the input and button components.

<CodeGroup
  options={[
    {
      name: "pnpm",
      lang: "bash",
      code: ["pnpm dlx npx shadcn-vue@latest add button input"].join("\n"),
    },
    {
      name: "npm",
      lang: "bash",
      code: ["npx npx shadcn-vue@latest add button input"].join("\n"),
    },
    {
      name: "yarn",
      lang: "bash",
      code: ["yarn dlx npx shadcn-vue@latest add button input"].join("\n"),
    },
    {
      name: "bun",
      lang: "bash",
      code: ["bunx npx shadcn-vue@latest add button input"].join("\n"),
    },
  ]}
/>

## Build a basic app

Amazing, now let’s make ourselves a quick landing page. Replace the following code in your `app.vue` file.

<CodeBlock name="app.vue" lang="vue">
  {`<template>
  <div class="dark space-x-2 max-w-sm mx-auto my-auto mt-20">
    <p class="text-center">Hey 👋</p>
    <h1 class="text-7xl font-extrabold text-center mb-4">Join my newsletter</h1>
    <p class="text-center mb-8">
      We talk about <i>amazing things</i>, delivered to your inbox
      <b>every Saturday</b>.
    </p>
    <div class="flex gap-2">
      <Input type="email" placeholder="Your email" />
      <Button>Join newsletter</Button>
    </div>
  </div>
</template>`}
</CodeBlock>

This should all do the job! We now need an endpoint to send the signup request to, along with the users actual email address.

## Backend with Resend

Now we can implement Resend's logic. First, [create an account with Resend](https://resend.com/signup) and [get your API key](https://resend.com/docs/dashboard/api-keys/introduction). Once you have your API key, create a `.env` file at the root of your directory and add it here.

<CodeBlock name=".env" lang="bash">
  {`RESEND_API_KEY=your_key_goes_here`}
</CodeBlock>

Now we need to create an endpoint that will handle adding the email address to our resend audience and sending them a thankyou email.

Create a new file in `server/api/newsletter.post.ts` and add the following code.

<CodeBlock name="newsletter.post.ts" lang="ts">
  {
`import { defineEventHandler, readBody, H3Event } from "h3";
import { Resend } from "resend";

const resend = new Resend(process.env.RESEND_API_KEY);

interface SubscribeRequestBody {
  email: string;
}

export default defineEventHandler(async (event: H3Event) => {
  try {
    const { email } = await readBody<SubscribeRequestBody>(event);

    if (!email || !email.includes("@")) {
      return {
        statusCode: 400,
        body: "Invalid email address",
      };
    }

    const { error: contactError } = await resend.contacts.create({
      email,
      unsubscribed: false,
      audienceId: process.env.RESEND_AUDIENCE_ID as string,
    });

    if (contactError) {
      return {
        statusCode: 400,
        body: contactError.message,
      };
    }

    await resend.emails.send({
      from: "onboarding@resend.dev",
      to: email,
      subject: "Hello",
      html: "<p>Thank you for joining my newsletter</p>",
    });

    return {
      statusCode: 200,
      body: "You have been added to the newsletter",
    };
  } catch (err: any) {
    return {
      statusCode: 500,
      body: err?.message || "Internal Server Error",
    };
  }
});`}
</CodeBlock>

Finally all that is left to do is to hook the frontend up to this api call

<CodeBlock name="app.vue" lang="ts">
  {`const handleSubmitNewsletter = async (email: string) => {
    const response = await $fetch("/api/newsletter", {
      method: "POST",
      body: { email },
    });

    if (response.statusCode === 200) {
      toast(response.body);
    } else {
      toast("Error", {
        description: response.body || "An error occurred",
      });
    }
};`}
</CodeBlock>


Now just add this function to your current frontend

<CodeBlock name="app.vue" lang="vue">
  {`<script lang="ts" setup>
const email = ref<string | null>(null);
const handleSubmitNewsletter = async (email: string) => {
  const response = await $fetch("/api/newsletter", {
    method: "POST",
    body: { email },
  });

  if (response.statusCode === 200) {
    toast(response.body);
  } else {
    toast("Error", {
      description: response.body || "An error occurred",
    });
  }
};
</script>

<template>
  <div
    class="space-x-2 max-w-md mx-auto my-auto mt-20 border border-border px-6 py-12  rounded-lg"
  >
    <p class="pl-2">Hey 👋</p>
    <h1 class="text-3xl font-extrabold  mb-2">Join my newsletter!</h1>
    <p class=" mb-16">
      We talk about <i>amazing things</i>, delivered to your inbox
      <b>every Saturday</b>.
    </p>
    <div class="flex gap-2 ">
      <Input type="email" placeholder="Your email" v-model="email" />
      <Button @click="handleSubmitNewsletter(email)">Join newsletter</Button>
    </div>
  </div>
</template>`}
</CodeBlock>


And we have a working newsletter!
