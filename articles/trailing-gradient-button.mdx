...
Ok, so I have been getting really inspired recently by the [raycast landing page](https://raycast.com). The whole site is really beautiful, but there is one thing I really wanted to learn how to make: this cool button effect. It took me a lot longer than I want to admit to get this working, and the process was super fascinating. Let's dive in!
...
Now we can get into the fun stuff: the animation. There are two options for animating this — JavaScript or CSS — and I used CSS via the [`@property`](https://developer.mozilla.org/en-US/docs/Web/CSS/@property) rule from the [Houdini API](https://developer.mozilla.org/en-US/docs/Web/API/Houdini_APIs). It’s not supported on every browser, so double check if you’re deploying this in production.
