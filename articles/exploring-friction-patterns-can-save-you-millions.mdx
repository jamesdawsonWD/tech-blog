---
title: "Friction patterns can save you millions"
...
Losing tens of millions of dollars through a user interface sounds absurd at first.

But when you build financial products, especially permissionless ones, the interface is not a thin cosmetic layer. It is the final interpreter between a human intention and an irreversible machine action. And when the stakes are high enough, a small weakness in that interpretation can become catastrophically expensive.

Recently, a user attempted to buy AAVE using **$50 million USDT** through the Aave interface. According to Stani, the interface did warn about extraordinary slippage and required the user to tick a confirmation checkbox before proceeding. The user accepted that warning, completed the swap on mobile, and received a disastrously bad outcome: **only 324 AAVE** in return.

That detail matters.

Because if a user can acknowledge a warning this large and still proceed into a result this far from what they likely intended, then the problem is bigger than whether a checkbox technically existed. The problem is whether the friction pattern was strong enough to create true understanding.

That is the part I think product designers, especially in crypto, need to explore much more seriously.
